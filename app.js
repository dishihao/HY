    ...
    const STORAGE_KEY = "hy-mint-assay-calculator-v1";
    const DEFAULTS = {
      recordNo: "TS-45-06401-a",
      version: "03",
      balanceModel: "AVW120D",
      balanceNo: "HY-TP-09",
      gcModel: "8860",
      gcNo: "HY-QX-01",
      rsdLimit: "2.0",
    ...
    function calculate() {
      ...
      const contentRaw1 = calculateContent(sampleAvg1, concentration, getF("dilution1"), stdInjection, stdAvg, getF("weight1"), getF("moisture1"), getF("sampleInjection1"));
      const contentRaw2 = calculateContent(sampleAvg2, concentration, getF("dilution2"), stdInjection, stdAvg, getF("weight2"), getF("moisture2", "moisture1"), getF("sampleInjection2"));
      ...
      $("specText").value = specRaw ? `本品按干燥品计算，含薄荷脑${relationText}${specRaw}%。` : "请输入标准规定。";

      let contentPass = null;
      if (avgShown && spec) contentPass = relation === "max" ? compare(avgShown, spec) <= 0 : compare(avgShown, spec) >= 0;
      ...
      if (contentPass != null && rsdPass != null) overallState = contentPass && rsdPass ? ["ok", "符合规定"] : ["bad", "不符合规定"];
    ...
    function collectState() { return Object.fromEntries(fields.map(el => [el.dataset.key, el.value])); }
    ...
    fields.forEach(el => {
      el.addEventListener("input", () => { calculate(); scheduleSave(); });
      el.addEventListener("change", () => { calculate(); scheduleSave(); });
    });

    $("exampleBtn").addEventListener("click", () => { applyState(EXAMPLE); saveNow(); toast("已载入纸质记录示例"); });

    $("resetBtn").addEventListener("click", () => {
      if (!confirm("确认清空当前计算数据？仪器型号、编号和默认规则会保留。")) return;
      applyState(DEFAULTS);
      saveNow();
      toast("已清空");
    });
